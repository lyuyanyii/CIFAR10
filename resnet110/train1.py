...
from network1 import make_network
import numpy as np
			...
			with control(io = [va_p]):
				...
				while i <= TOT_IT:
					...
					if i > 10000 and loss > 1.5:
						...
						env.save_checkpoint("resnet110.data1.boom")
					his.append([loss, acc])
					...
					if i == 64000:
						...
						env.save_checkpoint("resnet110.data1.64000")
					if i % (EPOCH_NUM) == 0:
						epoch += 1
						...
						print("train_acc = {}".format(np.mean(batch_acc)))
						batch_acc = []
						b = time.time()
						b = b + (b - a) / i * TOT_IT
						...
						if acc > max_acc and i > 64000:
							max_acc = acc
							env.save_checkpoint("resnet110.data1.bestmodel")
						env.save_checkpoint("resnet110.data1")
						print("**************************")
						import pickle
